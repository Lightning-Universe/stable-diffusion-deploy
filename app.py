...
class MuseFlow(L.LightningFlow):
    ...
    def __init__(
        ...
        for i in range(initial_num_workers):
            work = StableDiffusionServe(
                cloud_compute=L.CloudCompute(gpu_type, disk_size=30), cache_calls=True, parallel=True
            )
            self.add_work(work)
    ...
    def run(self):
        ...
        if all(model_serve.url for model_serve in self.model_servers) and not self.load_balancer_started:
            ...
            self.load_balancer.run([server.url for server in self.model_servers if server.is_model_ready])
