...
export const postDream = async (dream: string, highQuality: boolean, url: string) => {
  const headers = {
  ...
  return axios
    .post<{ image: string }>(url + '/api/predict', { prompt: dream, high_quality: highQuality }, config)
    .then(({ data }) => data)
    .catch(e => {
